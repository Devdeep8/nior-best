...
function SpotlightCard({
  ...
  return (
    <div
      ...
      className="tagline-item group relative overflow-hidden rounded-2xl border transition-all duration-700 flex flex-col justify-between min-h-[200px] sm:min-h-[240px] p-6 sm:p-8 lg:p-10 cursor-default w-full max-w-sm md:max-w-none mx-auto backdrop-blur-sm border-[#41050c]/10 bg-[#cecccc] hover:border-[#41050c]/30 hover:bg-[#cecccc]/90 hover:scale-[1.02] hover:-translate-y-2"
      style={{
    ...
    >
      ...
      <div
        ...
        style={{
          opacity,
          background: `radial-gradient(600px circle at ${position.x}px ${position.y}px, rgba(65, 5, 12, 0.08), transparent 40%)`,
        }}
      />
      ...
      <div
        ...
        style={{
          background: `radial-gradient(400px circle at ${position.x}px ${position.y}px, rgba(65, 5, 12, 0.12), transparent 60%)`,
        }}
      />

      {/* Top Section: ID Badge */}
      <div className="relative z-10 flex justify-between items-start w-full mb-6 sm:mb-8">
        <span className="font-mono text-sm tracking-wider transition-colors duration-500 text-[#41050c]/60 group-hover:text-[#41050c]">
          [{id}]
        </span>

        {/* Decorative small corner graphic */}
        <div className="w-2 h-2 border-t border-r transition-colors duration-500 border-[#41050c]/20 group-hover:border-[#41050c]/60" />
      </div>

      {/* Center Section: Typography */}
      <div className="relative z-10 flex flex-col items-start w-full select-none">
        <h3 className="text-2xl sm:text-3xl lg:text-[1.75rem] xl:text-[2.5rem] font-semibold tracking-tight lg:tracking-tighter text-[#41050c] overflow-hidden relative pb-1 w-full">
          ...
          <span className="absolute left-0 top-0 inline-block transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] text-[#41050c] whitespace-nowrap translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100">
            {title}
          </span>
        </h3>

        <p className="mt-3 sm:mt-4 text-xs sm:text-sm font-normal leading-relaxed max-w-[240px] transform transition-all duration-500 text-[#41050c] opacity-80 group-hover:opacity-100">
          {subtitle}
        </p>
      </div>

      {/* Bottom accent bar — hover only */}
      <div className="absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-[#41050c] to-transparent transition-transform duration-700 origin-center z-20 scale-x-0 group-hover:scale-x-100" />
    </div>
  );
}
